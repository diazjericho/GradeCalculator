package com.mariejuana.gradecalculator.ui.screens.dialog.delete.activity
...
class DeleteActivityDialog : DialogFragment() {
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        val categoryId = bundle!!.getString("deleteCategoryId").toString()
        val categoryName = bundle!!.getString("deleteCategoryName").toString()
        val activityId = bundle!!.getString("deleteActivityId").toString()
        val activityName = bundle!!.getString("deleteActivityName").toString()

        with(binding) {
            textName.text = "Delete ${activityName}?"
            textDetails.text = "Are you sure you want to delete '${activityName}'? " +
                    "This cannot be undone."
            buttonDelete.setText("Delete activity")

            buttonDelete.setOnClickListener {
                val coroutineContext = Job() + Dispatchers.IO
                val scope = CoroutineScope(coroutineContext + CoroutineName("deleteActivityDetails"))
                scope.launch(Dispatchers.IO) {
                    database.deleteActivity(categoryId, activityId)
                    withContext(Dispatchers.Main) {
                        Toast.makeText(activity, "Activity has been deleted!", Toast.LENGTH_LONG).show()
