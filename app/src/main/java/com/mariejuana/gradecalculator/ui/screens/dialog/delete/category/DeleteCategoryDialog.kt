package com.mariejuana.gradecalculator.ui.screens.dialog.delete.category
...
class DeleteCategoryDialog : DialogFragment() {
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        with(binding) {
            textName.text = "Delete ${categoryName}?"
            textDetails.text = "Are you sure you want to delete '${categoryName}'? " +
                    "This cannot be undone."
            buttonDelete.setText("Delete category")

            buttonDelete.setOnClickListener {
                val coroutineContext = Job() + Dispatchers.IO
                val scope = CoroutineScope(coroutineContext + CoroutineName("deleteCategoryDetails"))
                scope.launch(Dispatchers.IO) {
                    database.deleteCategory(subjectId, categoryId)
                    withContext(Dispatchers.Main) {
                        Toast.makeText(activity, "Category has been deleted!", Toast.LENGTH_LONG).show()
